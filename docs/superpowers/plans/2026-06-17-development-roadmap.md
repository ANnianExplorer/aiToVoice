...
## Phase 2: 前端 UI 重构（双主题：暗色 Spotify + 亮色苹果风）

**目标**：视觉体验从"能用"提升到"好用好看"，支持暗色/亮色双主题切换，去除通用 AI 生成感。

**主题策略**：
- **暗色主题 → Spotify 风格**：深黑背景 `#121212`、卡片 `#181818`、悬浮 `#282828`、绿色主色 `#1DB954`、紧凑布局、hover 渐变高亮、无衬线字体
- **亮色主题 → 苹果风**：纯白/浅灰背景、大圆角（12-16px）、毛玻璃效果（`backdrop-filter: blur`）、充足留白、SF Pro 字体风格、柔和阴影、极简线条

| 任务 | 说明 | 工具 |
|------|------|------|
| 2.1 主题系统 + 设计语言 | CSS 变量体系、暗色/亮色 token 定义、`useTheme` hook、主题切换动画（过渡 0.3s）、Ant Design ConfigProvider 双主题配置 | `frontend-design` skill |
| 2.2 首页重设计 | 歌曲卡片网格、推荐区域、入场动效、骨架屏加载；暗色紧凑卡片 vs 亮色大白卡片 | `frontend-design` skill |
| 2.3 播放器重设计 | 迷你播放器 + 全屏播放器、歌词同步显示、封面旋转动画、进度条美化；暗色底栏 vs 亮色毛玻璃底栏 | `frontend-design` skill |
| 2.4 侧边栏 & 导航 | 选中态高亮、折叠态、路由过渡动画、活跃指示器；暗色深黑侧栏 vs 亮色半透明侧栏 | `frontend-design` skill |
...
| v3.0 | Phase 2 | UI 全面重构（暗色 Spotify + 亮色苹果风双主题） |
